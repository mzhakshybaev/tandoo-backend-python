...
from appconf import API_ZAKUPKI
...
def portal_post(url, data=None, timeout=600):
    if not data:
        data = {}
    headers = {'Content-Type': 'application/json', 'Accept': 'application/json'}
    post_data = json.dumps(data, cls=JSONEncoderCore)
    try:
        response = requests.post(API_ZAKUPKI + url.strip(), data=post_data, headers=headers, timeout=timeout)
        if response.status_code == requests.codes.ok:
            data = json.loads(response.text)
            return data
        raise CbsException(messages.GENERIC_ERROR,
                           'EDS REQUEST ERROR {}: {}'.format(response.status_code, response.text))
    except (requests.Timeout, requests.ConnectionError):
        raise CbsException(messages.GENERIC_ERROR, 'EDS REQUEST TIMEOUT')


def portal_get(url, data=None, timeout=600):
    if not data:
        data = {}
    headers = {'Content-Type': 'application/json', 'Accept': 'application/json'}
    ...
    post_data = json.dumps(data, cls=JSONEncoderCore)
    try:
        response = requests.get(API_ZAKUPKI + url.strip(), data=post_data, headers=headers, timeout=timeout)

        if response.status_code == 200:
            data = json.loads(response.text)
            return data
        return {'result': response.status_code, 'message': response.text}
    except (requests.Timeout, requests.ConnectionError):
        return {'result': -1, 'message': 'Time Out'}
