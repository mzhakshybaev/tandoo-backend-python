...
export default class News extends Component {
    ...
    {Header: "Дата создания", accessor: "created_from", Cell: ({value}) => formatDateTime(value)},
    {
      ...
      Cell: ({value, row}) =>
        <Switcher
          size={"sm"}
          checked={value}
          onChange={value => this.onSwitchClick(row._id, value)}/>,
      width: 50,
      filterable: false
    },
    {
      Cell: ({row}) =>
        <Fragment>
          <AppButton size={"sm mr-2"}
                     ...
                     onClick={() => this.onItemEditClick(row._id)}>
            <span className={"fa fa-edit"}/>
          </AppButton>

          <AppButton size={"sm"} onClick={() => this.onItemViewClick(row._id)}>
  ...
  async load() {
    let news = await newsApi.getAllNewsList();
    this.setState({news})
  }
  ...
  onSwitchClick = async (_id, value) => {
    let params = {_id, active: value};
    await newsApi.switchActive(params);
    this.load()
  };
  ...
  render() {
    let {news, showList, itemEdit} = this.state;
    const {t}= this.props;
    return (
      ...
      <div>
        ...
        <Fragment>
          <h5>{t('Новости')}</h5>
          <AppTable data={news} columns={this.tableColumns} showRowNumbers/>

          <div>
            <AppButton onClick={() => this.setState({showList: false})}>Создать</AppButton>
          </div>
        </Fragment>}

        {!showList &&
        <NewsBuilder item={itemEdit} onExit={this.onExitBuilder}/>}
      </div>
    )
  }
}
...
class NewsBuilder extends Component {
  ...
  onClickSave = async () => {
    ...
    try {
      await newsApi.saveNews(params);

      showSuccess("Новость сохранена");
    ...
    return (
      <div className="news-editor">
        <Col>
          ...
          <Row>
            <h5>Новость (русский)</h5>
          </Row>

          <Row className="mt-2">
            <Input value={title_ru} placeholder="Teма новости" onChange={e => onTitleChange('ru', e.target.value)}/>
          </Row>
          ...
          <Row className="mt-2">
            <h5>Новость (кыргызча)</h5>
          </Row>

          <Row className="mt-2">
            <Input value={title_kg} placeholder="Teма новости" onChange={e => onTitleChange('kg', e.target.value)}/>
          </Row>
          ...
          <Row className="mt-2">
            <h5>Новость (english)</h5>
          </Row>

          <Row className="mt-2">
            <Input value={title_en} placeholder="Teма новости" onChange={e => onTitleChange('en', e.target.value)}/>
          </Row>
